...
export class NewsService {
  ...
  searchNews(
    ...
    return this.getHomeFeed(filters, preferredCategories).pipe(map((feed) => feed.articles));
  }
  ...
  private groupArticlesByCategory(articles: NewsArticle[]): NewsStore {
    const emptyStore = this.categories.reduce((store, category) => {
      store[category] = [];
      return store;
    }, {} as NewsStore);

    for (const article of articles) {
      const categories = article.matchedCategories?.length
        ? article.matchedCategories
        : [article.category];

      for (const category of categories) {
        emptyStore[category].push(article);
      }
    }

    return this.categories.reduce((store, category) => {
      store[category] = this.sortByNewest(this.dedupeArticles(store[category]));
      return store;
    }, emptyStore);
  }
