...
class NewsAssistantService:
    ...
    def get_runtime_status(self) -> dict[str, Any]:
        return self.ollama_client.runtime_status()
...
""".strip()
    ...
    def _answer_general_question(self, message: str) -> dict[str, Any]:
        try:
            ...
            answer = (
                "I can handle general questions too, but the local AI assistant is unavailable right now. "
                "Please make sure Ollama is running and that a Qwen3 model is installed, then try again."
            )
